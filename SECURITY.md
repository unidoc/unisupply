...
`vuln.go.dev`, `api.osv.dev`, `services.nvd.nist.gov`), threat-intelligence
feeds (`api.first.org` for EPSS, `www.cisa.gov` for the CISA KEV catalog),
always-on GitHub API calls (`api.github.com`), an optional user-supplied
Trust Index URL, and
