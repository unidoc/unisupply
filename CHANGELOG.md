...
- **Documentation drift in the network contract fails the build.** A new
  ...
  the table does not document, and no documented row goes unexercised. Rows
  that cannot be driven in-process (`vuln.go.dev`, `cloud.unidoc.io`, the
  ...
  summary is checked against the table. No request leaves the machine: the
  recorder rewrites each request to a local stub server.
